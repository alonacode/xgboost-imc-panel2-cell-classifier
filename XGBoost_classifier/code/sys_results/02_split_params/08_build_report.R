#!/usr/bin/env Rscript
# 08_build_report.R  (split-parameter experiment)
# Inverse of 01_model_params: the MODEL is fixed at the guardrail-best params (M9) and the
# tier-aware train/test SPLIT is varied (S1-S10). Each split scenario is evaluated end-to-end
# (image-grouped xgb.cv to pick nrounds_best, final fit, held-out test metrics + confusion matrix).
# Writes tuning_results_split.csv + report_cms_split.rds (suffixed so model artifacts are untouched).
#
# NOTE: every split scenario produces a DIFFERENT held-out test set, so cross-scenario metric
# differences conflate model quality with which images land in test. S1/S9/S10 are the same config
# at different seeds and exist to quantify that split-noise floor.
...
# ---------------------------------------------------------------------------
# 4. Split grid (S1-S10) — analog of xgb_grid, but varying the split knobs.
# ---------------------------------------------------------------------------
split_grid <- tibble::tribble(
  ~frac_crit, ~frac_med, ~frac_non, ~w_crit, ~w_med, ~w_non, ~min_crit, ~seed, ~note,
  0.30,       0.25,      0.20,      40,      10,     1,      8,         42,    "baseline (main split)",
  0.40,       0.25,      0.20,      40,      10,     1,      8,         42,    "more critical test (0.40)",
  0.20,       0.20,      0.20,      40,      10,     1,      8,         42,    "uniform 0.20",
  0.25,       0.25,      0.25,      40,      10,     1,      8,         42,    "uniform 0.25",
  0.30,       0.25,      0.20,      100,     10,     1,      8,         42,    "stronger critical weight (100)",
  0.30,       0.25,      0.20,      1,       1,      1,      8,         42,    "equal weights (no priority)",
  0.30,       0.25,      0.20,      40,      10,     1,      15,        42,    "higher floor (min=15)",
  0.30,       0.25,      0.20,      40,      10,     1,      0,         42,    "no floor (min=0)",
  0.30,       0.25,      0.20,      40,      10,     1,      8,         7,     "baseline, seed=7",
  0.30,       0.25,      0.20,      40,      10,     1,      8,         123,   "baseline, seed=123")

scenarios <- lapply(seq_len(nrow(split_grid)), function(i) {
  g <- split_grid[i, ]
  list(label = paste0("split_", i, " (", g$note, ")"), split_id = paste0("S", i),
       model_id = paste0("S", i), nrounds = 500L, clean_cfg = clean_cfg_baseline, xgb = best_xgb,
       split_cfg = list(
         tier_test_frac = c(critical = g$frac_crit, medium = g$frac_med, non_critical = g$frac_non),
         tier_weight    = c(critical = g$w_crit,    medium = g$w_med,    non_critical = g$w_non),
...
write.csv(results, file.path(out_dir, "tuning_results_split.csv"), row.names = FALSE)
saveRDS(list(cm_store = cm_store, split_store = split_store, results = results,
        ...
        file.path(out_dir, "report_cms_split.rds"))
message("Wrote tuning_results_split.csv (", nrow(results), " rows) and report_cms_split.rds. ",
